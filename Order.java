...
public class Order implements Printable {
    ...
    private HashMap<MenuItem, Integer> itemsQuantity;
    ...
    public void add(final MenuItem item, final int quantity) {
        ...
        itemsQuantity.put(item, itemsQuantity.getOrDefault(item, 0) + quantity);

        if (itemsQuantity.get(item) <= 0) {
    ...
    public BigDecimal getSubTotal() {
        ...
        for (MenuItem item : itemsQuantity.keySet()) {
            subTotal = subTotal.add(item.getPrice().multiply(BigDecimal.valueOf(itemsQuantity.get(item))));
        }
    ...
    public String toString() {
        ...
        for (MenuItem item : itemsQuantity.keySet()) {
            ...
            result.append(String.format("%s x%d: $%.2f%n", item.getName(), quantity, itemTotal));
        }

        result.append("\nSubtotal: $").append(getSubTotal());
        result.append("\nTaxes: $").append(getTaxes().setScale(2, BigDecimal.ROUND_HALF_EVEN));
        result.append("\nTotal: $").append(getTotal().setScale(2, BigDecimal.ROUND_HALF_EVEN));
    ...
    public void update(final MenuItem item, final int quantity) {
        ...
        // Update the quantity of a MenuItem in the Order
        itemsQuantity.put(item, quantity);

        // Remove the item from the order if the quantity is 0 or negative
        if (quantity <= 0) {
            itemsQuantity.remove(item);
        }
    }
